...
class PDFDocument {
  ...
  fetch(ref: Ref, recursive?: boolean) {
    ...
    if (xref_entry.type == 2) {
      ...
      if (!object_stream) {
        const object_stream_obj = this.fetch(new Ref(xref_entry.offset, 0), true);
...
class PDFReader {
  ...
  parse_hex_string() {
    ...
    return hex_str.replace(/([0-9A-Fa-f]{2})/g, (_, args) => String.fromCharCode(parseInt(args[0], 16)));
  }
  ...
  static DELIM_CHARS = [0, 9, 13, 10, 32, 40, 41, 60, 62, 91, 93, 123, 125, 47, 37]
  ...
  parse_stream_content(map: map) {
    ...
    if (map.Filter) {
      const filters = (map.Filter instanceof Array) ? map.Filter : [map.Filter];
      const params = (map.DecodeParms instanceof Array) ? map.DecodeParms : [map.DecodeParms];
  ...
  parse_xref() {
    ...
    const total_count = obj.content.length / (type_length + offset_length + gen_length);
...
class PDFObjects {
  constructor(
    doc: PDFDocument,
    public entries = doc.get_xref_entries()
  ) {
...
function visit_acroform_fields(doc: PDFDocument, callback: Function) {
 ...
 debug? console.log('find',n):''
     if (n.map && n.map.Kids && n.map.Opt && n.map.FT && (n.map.FT.name === 'Btn') ) {
        ...
        callback(n);
      }else if (n.map && n.map.Kids &&n.map.FT&& (n.map.FT.name === 'Tx')  ) { //muilt show text
        callback(n);
...
function modify_xfa(doc: PDFDocument, objects: PDFObjects, out: BytesIO, index: string, callback: Function) {
  if (!doc.acroForm) {
    return;
  }
